document.addEventListener("DOMContentLoaded", () => {
    ...
    const groupMusicSettings = document.getElementById("group-music-settings");
    ...
    function updateEngineStatus() {
        if (currentMode === "image") {
            ...
            const hfKeyVal = localStorage.getItem("key_huggingface");
            if (hfKeyVal) {
                apiStatusDot.className = "indicator-dot";
                apiStatusText.innerText = "Using MusicGen (Hugging Face Key Configured)";
            } else {
                apiStatusDot.className = "indicator-dot warning";
                apiStatusText.innerText = "Hugging Face Token missing! Configure in Settings.";
            }
    ...
    function setMode(mode) {
        ...
        if (mode === "image") {
            ...
            modeMusicBtn.classList.add("active");
            groupImageSettings.classList.add("hidden");
            groupMusicSettings.classList.remove("hidden");
            groupVoiceSettings.classList.add("hidden");
            generateTypeLabel.innerText = "Music";
            promptInput.placeholder = "Upbeat lofi electronic beats for coding, synthesizer chords, slow tempo...";
        } else if (mode === "speech") {
    ...
    // 2. Music Generation Request
    async function generateMusic(prompt) {
        setCanvasLoading(true, "Synthesizing audio waves (usually takes 10-20 seconds)...");
        
        const api_key = localStorage.getItem("key_huggingface") || "";
        if (!api_key) {
            showToast("Please configure your Hugging Face Token in settings to generate AI Music!", "error");
            setCanvasLoading(false);
            return;
        }
        ...
        try {
            ...
            const data = await response.json();
            if (response.ok) {
                displayOutput("audio", data.url, prompt, "AI Generated Melody");
                showToast("Music track generated successfully!", "success");
            } else {
