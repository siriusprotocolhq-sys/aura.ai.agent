...
# Resolve paths dynamically to support both root-level and frontend-folder setups securely
BASE_DIR = os.path.dirname(__file__)

@app.get("/static/style.css")
def get_style():
    for path in [os.path.join(BASE_DIR, "frontend", "style.css"), os.path.join(BASE_DIR, "style.css")]:
...
def get_script():
    for path in [os.path.join(BASE_DIR, "frontend", "script.js"), os.path.join(BASE_DIR, "script.js")]:
        if os.path.exists(path):
...
@app.get("/")
def read_root():
    for path in [os.path.join(BASE_DIR, "frontend", "index.html"), os.path.join(BASE_DIR, "index.html")]:
        if os.path.exists(path):
            return FileResponse(path)
    return JSONResponse(
        content={"message": "FastAPI is running! index.html not found. Please upload index.html to your GitHub repository."},
        status_code=200
    )
