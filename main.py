...
def generate_image(req: ImageGenerateRequest):
    ...
    if provider == "pollinations":
        ...
        if not api_key:
            raise HTTPException(status_code=400, detail="Hugging Face API key is required. Configure it in Settings.")
...
def generate_music(req: MusicGenerateRequest):
    prompt = req.prompt.strip()
    api_key = req.api_key or os.getenv("HUGGINGFACE_API_KEY")
    
    if not api_key:
        raise HTTPException(status_code=400, detail="Hugging Face API key is required for AI Music. Add it in Settings.")
        
    try:
        # Call Hugging Face Inference API for facebook/musicgen-small
        ...
        if response.status_code == 200:
            ...
            return {"url": f"/outputs/{filename}", "provider": "huggingface (MusicGen)"}
        elif response.status_code == 533 or (response.status_code == 503 and "loading" in response.text):
            # Model is loading
            res_json = response.json()
            est_time = res_json.get("estimated_time", 20.0)
            raise HTTPException(
                status_code=503, 
                detail=f"Hugging Face music model is currently loading on their servers. Please try again in {int(est_time)} seconds."
            )
        else:
            error_msg = f"Inference API returned HTTP {response.status_code}"
            try:
